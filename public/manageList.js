...
function renderlistsTable(listsData) {
  ...
  paginatedData.forEach(list => {
    const createdDate = toCentralISOString(list.createdAt);
...
function openViewModal(list) {
  ...
  document.getElementById("viewlistCreated").textContent = toCentralISOString(list.createdAt);
...
function openEditModal(list) {
  ...
  document.getElementById("editlistId").value = list.id.toLowerCase();
  document.getElementById("editlistId").disabled = true;
...
async function savelistChanges() {
  ...
  const listId = (document.getElementById("editlistId").value).trim().toLowerCase();
