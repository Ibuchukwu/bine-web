import { createPaymentModal, initiatePayment, startExpiryTimer, startPaymentPolling } from "./portalPay.js";
...
regnoInput.addEventListener('keypress', getProfile());
// Event listeners
    ...
    if (regno.length == regLen){
      ...
      try {
        ...
        if (!response.ok) {
          ...
          return;
        }if(!data.details.details.profileVerified){
        ...
        if(pendingPayment){
          const expireAt = (pendingPayment.expireAt - Date.now())/60000;
          const expiry = Math.floor(expireAt); 
          if (expiry > 3){
          runWindow("open",`You have an incomplete payment that will expire in ${expiry}.<br> would you like to complete it now? <br>
            <div>
              <button type="button" onclick="resumePayment(pendingPayment, expireAt * 60)" class="btn btn-primary">Complete Payment</button>
              <button type="button" onclick="runWindow('close')" class="btn btn-outline">Discard Payment</button>
            </div>`);
            }
        }
        ...
        detailsElement.innerHTML = `
          <div class="profile-card">
            ...
            <div class="profile-detail">
              ...
              <span class="detail-value">${data.details.details.name || 'Not available'}</span>
            </div>
            <div class="profile-detail">
              <span class="detail-label">Department:</span>
              <span class="detail-value">${data.details.details.departmentName || 'Not available'}</span>
            </div>
            <div class="profile-detail">
              <span class="detail-label">Class:</span>
              <span class="detail-value">${(data.details.details.classId).toUpperCase() || 'Not available'}</span>
            </div>
          </div>
        `;
        profileDetails = data.details.details;
        //Hide Dues
        duesContainer.style.display = 'none';
        totalSection.style.display = 'none';
        showDuesBtn.innerHTML = "Show Dues";
...
async function resumePayment(pendingPayment, expiry) {
  runWindow("close",);
  const modalContainer = document.getElementById('payment-modal-container');
  modalContainer.querySelector('.payment-modal').innerHTML = createPaymentModal(pendingPayment.accountDetails);
  modalContainer.classList.remove('hidden');
  startExpiryTimer(expiry);
  startPaymentPolling(pendingPayment.accountDetails.accountNumber);
}
...
function createDueItem(due) {
  ...
  if (!due.paid) {
    ...
    paymentDetails = `
      ...
      <p><strong>Serial No:</strong> <span>${due.paymentDetails.serialNumber || "NaN"}</span></p>
  ...
  if(!due.paid) dueHeader.appendChild(dueSelector);
  dueHeader.appendChild(dueCode);
  if(due.isOneTime) dueCode.appendChild(oneTimeBadge);
  ...
  dueDetails.innerHTML = `
      ...
      <p><strong>Due Name:</strong> <span>${due.name}</span></p>
      ...
      <p><strong>Due description:</strong><div>${due.description}</div></p>
